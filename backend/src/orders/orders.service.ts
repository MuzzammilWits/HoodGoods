  ...
  async createOrder(createOrderDto: CreateOrderDto, buyerUserId: string): Promise<Order> {
    ...
    try {
      ...
      for (const storeId of storeIds) {
        ...
        await queryRunner.manager.save(SellerOrderItem, sellerOrderItemsToCreate);
    ...
    if (savedOrder) {
       this.logger.log(`Service: Attempting final fetch for Order ID: ${savedOrder.orderId}`); // Use property name
       try {
