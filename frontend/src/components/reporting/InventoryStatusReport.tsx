...
const InventoryStatusReport: React.FC = () => {
    ...
    const pieChartContainerRef = useRef<HTMLElement>(null); // Use HTMLElement for semantic elements
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            if (pieChartContainerRef.current) {
                ...
                const canvas = await html2canvas(pieChartContainerRef.current, {
    ...
    if (loading) return <p className="loading-message">Loading inventory report...</p>;
    if (error) return <p className="error-message">Error: {error}</p>;
    if (!reportData) return <p className="info-message">No inventory data available.</p>;

    return (
        <article className="inventory-status-report">
            <header>
            ...
            {pieChartData && (
                ...
                >
                    <figcaption>Stock Breakdown (Total Products: {reportData.stockBreakdown.totalProducts})</figcaption>
                    <Pie
                        data={pieChartData}
                        options={{
                            responsive: true,
            ...
            <section className="inventory-sections-grid">
                <section className="inventory-section low-stock-section">
                ...
                <section className="inventory-section out-of-stock-section">
            ...
            <section className="inventory-section full-inventory-section">
                <h3>Full Inventory List</h3>
                <form className="filters">
                    <label htmlFor="search-term" className="visually-hidden">Search by product name</label>
                    <input
                        id="search-term"
                        type="text"
