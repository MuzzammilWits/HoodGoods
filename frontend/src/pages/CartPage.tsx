...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    // Fetches delivery options for unique stores in the cart.
  ...
  useEffect(() => {
    // Cleans up payment timers and Yoco popup on component unmount.
    return () => {
  ...
  const handlePayment = async () => {
    // Validates inputs before initiating payment.
  ...
  // Displays loading messages for different states.
        ...
        <section className="payment-section">
          ...
          {isProcessingPayment && paymentTimeLeft !== null && (
            <p className="payment-timer" style={{
              ...
              Payment gate is <strong>open</strong> for: <time style={{fontVariantNumeric: 'tabular-nums'}}>0:{paymentTimeLeft.toString().padStart(2, '0')}</time><br/>
              <em style={{fontSize: '1rem', fontWeight: 500, display: 'block', marginTop: '0.3rem'}}>
